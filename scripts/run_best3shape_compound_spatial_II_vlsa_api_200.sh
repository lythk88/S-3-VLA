...
export PYTHONPATH="$SOURCE_ROOT/main:$SOURCE_ROOT/openpi/src:$SOURCE_ROOT/openpi/packages/openpi-client/src:$RUNTIME_ROOT/.deps_qwen:$RUNTIME_ROOT/safelibero:$RUNTIME_ROOT/GroundingDINO"
export LIBERO_CONFIG_PATH="$RUNTIME_ROOT/evaluation/libero_eval_config_local"
...
COMMON=(
  ...
  --action-expert-critic-run-dir "$RUNTIME_ROOT/Safety-value-function/success_critic_v1"
  --policy-checkpoint-dir /root/.cache/openpi/openpi-assets/checkpoints/pi05_libero/pi05_libero
  --groundingdino-config-path "$RUNTIME_ROOT/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py"
  --groundingdino-checkpoint-path "$RUNTIME_ROOT/GroundingDINO/groundingdino_swint_ogc.pth"
