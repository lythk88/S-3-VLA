...
RUN_NAME=smcbf_z_top_zero_contact_audit_80
...
"$ROOT/.venv/bin/python" "$ROOT/main/main_aegis.py" \
  --task-suite-name safelibero_object --safety-level I \
  "${COMMON[@]}" >"$OUT/logs/object_I.log" 2>&1 &
object_pid=$!

"$ROOT/.venv/bin/python" "$ROOT/main/main_aegis.py" \
  --task-suite-name safelibero_spatial --safety-level II \
  "${COMMON[@]}" >"$OUT/logs/spatial_II.log" 2>&1 &
spatial_pid=$!

status=0
wait "$object_pid" || status=$?
wait "$spatial_pid" || status=$?
exit "$status"
